...
def main_phi(freq, h_tx, h_rx, plot=False, export=False, rho=1.):
...
def main_power_single_freq(freq, h_tx, h_rx, rho=1., plot=False, export=False):
    if not 0 < rho <= 1:
    ...
    _pr_d1 = to_decibel(rec_power(np.max(_crit_dist_interval), freq, h_tx, h_rx, G_ref=G_ref))
    ...
    if export:
        ...
        export_results(results, f"power_single-{freq:E}-t{h_tx:.1f}-r{h_rx:.1f}-rho{rho:.2f}.dat")
    return results
